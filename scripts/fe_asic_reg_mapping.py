class FE_ASIC_REG_MAPPING:
    ...
    def set_fe_chn(self, chn=0, sts=-1, snc=-1, sg=-1, st=-1, smn=-1, sbf=-1):
...
####sec_chip_global only sets a chip global register, the other registers remains as before
    def set_fe_global(self, slk = -1, stb = -1, s16=-1, slkh=-1, sdc=-1, sdacsw2=-1, sdacsw1=-1, sdac=-1):

        existing_settings = self.REGS[4]
...
####sec_chip sets registers of a whole chip, registers of the other chips remains as before
    def set_fe_chip(self,
                 sts=0, snc=0, sg=0, st=0, smn=0, sbf=0,
                 slk=0, stb=0, s16=0, slkh=0, sdc=0, sdacsw2=0, sdacsw1=0, sdac=0):
        for chn in range(16):
            self.set_fe_chn(chn, sts, snc, sg, st, smn, sbf)     

        self.set_fe_global (slk, stb, s16, slkh, sdc, sdacsw2, sdacsw1, sdac)

####sec_sbnd_board sets registers of a whole board 
    def set_fe_board(self, sts=0, snc=0, sg=0, st=0, smn=0, sbf=0, 
                       slk = 0, stb = 0, s16=0, slkh=0, sdc=0, sdacsw2=0, sdacsw1=0, sdac=0):
        self.set_fe_chip(sts, snc, sg, st, smn, sbf, slk, stb, s16, slkh, sdc, sdacsw2, sdacsw1, sdac)

        
    #__INIT__#
    def __init__(self):
	#declare board specific registers, only SNC is high by default
         self.REGS =[0x00008181, 0x00008181, 0x00008181, 0x00008181, 0x00008181]