...
#                print ("Register {} {}".format(i, hex(self.femb.read_reg(i))))
    ...
    def configAdcAsic(self, to_print = True): #cp 1/29/18 #helper
        ...
        for k in range(10):            
           ...
           #self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0)
            ...
            if (to_print == True): # Print to screen
                print ("\t FEMB_CONFIG--> CONFIG() -> Check ADC ASIC SPI")
...
#                wrong = True
    ...
    def configFeAsic(self, to_print = True):
        Feasic_regs = self.fe_reg.REGS
        #ADC ASIC SPI registers
        self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0x40)
        self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0)
        time.sleep(0.01)
        self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0x10)
        self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0)
        time.sleep(0.01)
        self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0x40)
        self.femb.write_reg ( self.REG_ASIC_SPIPROG, 0)
