...
class Analyze {
	...
	//Constants
        const unsigned int maxNumAsic = 4;
	const unsigned int maxNumChan = 16;
        const unsigned int maxNumSubrun = 1024;
	...
	TCanvas* c0;
	TGraph *gCh;
	std::vector<unsigned short> asicPackets[1024][4]; //subrun, asics
	std::vector<unsigned short> wfIn[1024][16]; //subrun, ch
	...
	while(line < asicPackets[subrun][asic].size() ){
		...
		for(int ch = 0 ; ch < maxNumChan ; ch++){
			...
			//int chNum = 16*asic + ch;
                        int chNum = ch; //for single ASIC tests ignore the ASIC # in the output data
