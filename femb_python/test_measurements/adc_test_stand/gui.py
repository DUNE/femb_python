...
class GUI_WINDOW(Frame):
    ...
    def define_test_details_column(self):
        ...
        self.prepare_button.grid(row=50,column=columnbase,columnspan=2,pady=30)

        # Adding electronics ID and read entry box
        self.current_label = Label(self,text="CH2 Current [A]:",width=25,state="disabled")
        self.current_label.grid(sticky=W,row=105,column=columnbase+0)
        ...
        self.start_button.grid(row=108,column=columnbase,columnspan=2,pady=30)
        self.reset_button = Button(self, text="Reset & Power-off", command=self.reset,width=25,bg="#FF8000")
        self.reset_button.grid(row=110,column=columnbase,columnspan=2)

        self.runid_label = Label(self, text="")
        self.runid_label.grid(row=112,column=columnbase,columnspan=2,pady=20)
    ...
    def done_preparing_board(self,params):
        ...
        for iSocket in range(self.config.NASICS):
            ...
            theseTestNames = ["configADC","configFE","sync"]
            if titles_made:
            ...
            for iTest, test in enumerate(testNames):
                ...
                color = "#FF0000"
                text = "FAIL"
                if resultdict[test][iSocket] is None:
                    color = self.bkg_color
                    text = "NULL"
                elif resultdict[test][iSocket]:
                    color = "#00CC00"
                    text = "OK"
                label = Label(self, text=text,width=4,bg=color)
        ...
        if anyPass:
            ...
            columnbase = 0
            rowbase = 150
            label = Label(self, text="No successfully setup chips",bg="#FF0000")
