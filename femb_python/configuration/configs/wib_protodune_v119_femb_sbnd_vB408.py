...
class FEMB_CONFIG(FEMB_CONFIG_BASE):
    ...
    def wib_switch(self):
        ...
        #For BNL testing
        iplist = ["192.168.121.1","192.168.121.20"]
    ...
    def configFeAsic(self):
        ...
        for chip in range(self.NASICS):
            for chn in range(self.NASICCH):
                ...
                chn_reg = ((sts&0x01)<<7) + ((snc&0x01)<<6) + ((sg&0x03)<<4) + ((st&0x03)<<2)  + ((smn&0x01)<<1) + ((sdf&0x01)<<0)
                print("chip " + str(chip) + " chn " + str(chn) + " register " + str(hex(chn_reg)))
                chn_reg_bool = []
                for j in range(8):
    ...
    def checkFirmwareVersion(self):
        ...
        if wibVerReg != 0x119 :
            print("Invalid WIB firmware version detected " + str(hex(wibVerReg)) + ", this configuration requires version 0x116")
            return False
        if fembVerReg != 0x408 :
            print("Invalid FEMB firmware version detected " + str(hex(fembVerReg)) + ", this configuration requires version 0x408")
            return False
